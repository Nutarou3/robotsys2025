...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="標準入力またはコマンドライン引数で与えられたジャンルリストから献立を確率的に提案します。",
    )
    ...
    parser.add_argument(
        ...
        nargs='*',
        default=[],
        help='献立候補とするジャンル名（複数指定可）。例: にく さかな 中華'
    )
...
def main():
    ...
    if not input_genres:
        sys.stderr.write("エラー: 有効なジャンルが検出されませんでした。献立候補となるジャンルを指定してください。\n")
        sys.exit(1)
