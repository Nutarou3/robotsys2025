...
# 終了ステータス 0 で、STDERRに「警告: 」の文字列を含むことを確認
run_test \
    "echo -e 'にく\n未登録ジャンル\nパスタ' | $COMMAND" \
...
# 終了ステータス 1 で、STDERRに「有効なジャンルが...」の文字列を含むことを確認
run_test \
    "echo '' | $COMMAND" \
    "[[ \$TEST_STATUS -eq 1 ]] && grep -q 'エラー: 有効なジャンルが標準入力から検出されませんでした。' \$ERR_FILE" \
    "入力なしで異常終了(1)することを確認 (エラーメッセージとステータス)" \
    "終了ステータス 1, STDERRに「有効なジャンルが...」を含む"

# 1-4. 標準入力をオプションのように使っていないことの確認 (減点回避)
# コマンド引数を与えた場合に、必ず異常終了(1)することを確認
run_test \
    "$COMMAND にく" \
